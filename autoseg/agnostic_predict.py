...
def predict(
    ...
    # get out_dataset names, array keys, requests
    out_ds_names = []
    out_keys = []
    in_keys = []
    ...
    for out_key,num_channels in keys[1].items():
        ...
        if write=="all" or write==True or out_key.lower().split('_')[-1] in write: 
    ...
    extra_unsqueeze = [gp.Unsqueeze([in_key]) if keys[0][str(in_key)] == 1 else gp.Unsqueeze([in_key])+gp.Squeeze([in_key]) for in_key in in_keys]
    ...
    pipeline += gp.torch.Predict(
            ...
            inputs = {k:v for k,v in enumerate(in_keys)},
